...
def forecast_index(index_csv_filename, forecast_horizon):
    ...
    fitted = model.fit(preproc_train)
    
    #FORECAST on test
    preproc_fore_test = fitted.predict(n_periods = test_cardinality)
    ...
    #MODEL FITTING on train+test 
    fitted = model.fit(preproc)
    
    #out-of-sample FORECAST
    preproc_fore = fitted.predict(n_periods = forecast_horizon)
